...
export class ValidationSrv {
  ...
  private async validate(folderId: any, name: string, existingErrorMessage: string) {
    ...
    const promises = [];
    promises.push(getBackendSrv().search({ type: hitTypes.FOLDER, folderIds: [folderId], query: name }));
    promises.push(getBackendSrv().search({ type: hitTypes.DASHBOARD, folderIds: [folderId], query: name }));
